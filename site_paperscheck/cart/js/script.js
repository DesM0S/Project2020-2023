...
const cardInfo = {
    cardName: 'Работай быстро и продуктивно',
    cardAfter: 'Тимофей Бурдаков',
    cardText: `Далеко-далеко за словесными горами в стране гласных и согласных живут
...
<div class="card">
<div class="front">
    <h3 class="card-name">${cardInfo.cardName}</h3>
</div>
<div class="back">
    <div class="card-back-header">
        <div class="name-after">Автор: <span>${cardInfo.cardAfter}</span></div>
        <div class="burger-menu">
            ...
            <div class="burger-ico display-none" id="burger-menu-share">
                ...
                <div class="menu-share">
                    <a href="${cardInfo.cardChareLink[0]}"><img src="img/ico/facebook.svg" alt=""></a>
                    <a href="${cardInfo.cardChareLink[1]}"><img src="img/ico/twitter.svg" alt=""></a>
                    <a href="${cardInfo.cardChareLink[2]}"><img src="img/ico/vk.svg" alt=""></a>
                    <a href="${cardInfo.cardChareLink[3]}"><img src="img/ico/instagram.svg" alt=""></a>
                </div>
            </div>
...
function burgerMenuCard(card, back) {
    ...
    if (classParent('.toggle-burger-menu')) {
        toggleBurgerMenu(card)

        // закритие меню при нажатии всне его
    } else if (!classParent('.burger-menu')) {
        closeBurgerMenu(card)

        //при клике добовление и удаление из избранного карточки
    } else if (classParent('#add-and-remove-card')) {
        const addCard = classParent('#add-and-remove-card').children[0];
        const removeCard = classParent('#add-and-remove-card').children[1];
        //добавление и удаление карточек из избранных
        if (addCard.className === 'display-none' || removeCard.className === 'display-none') {
...
function increaseFontSize(back) {
    const btnMinus = back.querySelector('#minus-text');
    const btnPlus = back.querySelector('#plus-text');
